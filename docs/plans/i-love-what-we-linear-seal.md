...
`docs/research/product-team-vs-openspec.md:100` reports "~284k subagent tokens" as the pipeline's cost.
...
- **OpenSpec.** Its schema engine is a real option for replacing the hand-rolled STATUS.md state machine, but reconciliation cost rounds to zero and the machine-validation win is item 3 at no dependency cost. Take the idea, not the tool.
