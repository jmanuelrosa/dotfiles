...
- **Opus weighs far more against the weekly allowance than Sonnet.** The committed default here is `"model": "opus"` at `effortLevel: high`, chosen deliberately after a sonnet default proved the wrong trade on this work; the saving has to come from session hygiene rather than from the model dial.
- **`/model` writes the session default back to `settings.json`**, which is a symlink into this repo, so switching model in one session silently rewrites the committed default and leaves the repo dirty. Check `git diff` on that file before committing, and treat an unexplained change there as a leftover rather than a decision.
...
- **Print mode is a token oracle, and it is cheaper than reading transcripts.** `claude -p "reply with exactly: ok" --output-format json` returns a `usage` block whose `input_tokens + cache_creation_input_tokens + cache_read_input_tokens` is the whole prompt, so re-running it with one thing removed attributes a `/context` line exactly: `--tools ""` for the tool set, `--disallowedTools <name>` or a `--settings` file with a `permissions.deny` entry for a single tool, `CLAUDE_CODE_DISABLE_CLAUDE_MDS=1` for the memory layer, `CLAUDE_CODE_DISABLE_AUTO_MEMORY=1` for the memory instruction block, `CLAUDE_CODE_DISABLE_BUNDLED_SKILLS=1` for the bundled skill frontmatter. Two traps: print mode ignores `ENABLE_TOOL_SEARCH`, so it reports the undeferred tool set rather than what an interactive session carries, and it never loads the claude.ai connectors, so interactive-only tools like `Artifact` cannot be measured this way at all.
...
- **Built-in tools that survive deferral cost their whole schema every turn**, called or not, and the largest ones are the least used. `System tools` was 17.6k here, of which 14.5k was five tools with a combined six invocations across 462 transcripts: `Workflow` 7.9k, `Artifact` ~3.6k, `ScheduleWakeup` 1.7k, `ReportFindings` 0.8k, `ShareOnboardingGuide` 0.5k. `Workflow` alone outweighed every tool actually in use put together. All five are listed by bare name in `permissions.deny` in `settings.json`, which strips the schema rather than refusing the call; `CLAUDE_CODE_DISABLE_WORKFLOWS` and `CLAUDE_CODE_DISABLE_ARTIFACT` do the same for two of them, but only the deny list covers all five in one place.
- **`ENABLE_TOOL_SEARCH=auto:N` is the alternative for a tool worth keeping but rarely used.** N is a percentage of the tool budget kept eager, `auto:0` defers everything and `100` turns deferral off, so lowering N pushes more schemas behind `ToolSearch` instead of removing them. Coarse: it is the only knob, and it cannot be aimed at one tool.
