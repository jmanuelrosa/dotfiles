...
- `roles/ai/` — AI tooling (Claude Code, Gemini, Pi). Pi runs over Claude's payload rather than a copy of it: `AGENTS.md` and the global skills are shared by symlink, project skills reach it through the `.agents/skills` link `claude-kit` maintains, and Claude's enforcement hooks run on Pi's tool calls via `files/pi/extensions/guardrails.ts`. Pi's own system prompt file is `APPEND_SYSTEM.md`, since a `SYSTEM.md` would replace the harness prompt instead of adding to it. See [the pi harness](docs/internals/pi-harness.md).
