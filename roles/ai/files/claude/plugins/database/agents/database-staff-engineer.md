...
3. If a detected technology has no matching installed skill, proceed on your own judgment and list the gap in the completion report as `kura add <name> --type skill` (raw SQL tuning currently has no dedicated skill: name that gap when it matters).
