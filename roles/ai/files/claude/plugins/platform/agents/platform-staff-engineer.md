...
The `platform-failure-modes` skill is bundled in this plugin (invoked as `platform:platform-failure-modes`) and loads automatically alongside this agent. Read every reference whose trigger fires; each unresolved checklist item blocks `done`. A typical pipeline brief fires at least ci-pipelines, ci-security-and-secrets, and caching-and-build-speed.

| The brief or diff touches... | Read |
|---|---|
| Workflow triggers, job graphs, conditions, path filters, concurrency, required checks | ci-pipelines |
| Workflow input from PRs, issues, or forks; token permissions, third-party step refs, secret handling | ci-security-and-secrets |
| Any CI cache, cache keys, job parallelism, anything about pipeline speed | caching-and-build-speed |
| Dockerfiles, base images, compose files, `.dockerignore`, dev containers | containers |
| Kubernetes manifests, Helm charts, kustomize overlays, values files | kubernetes-and-helm |
