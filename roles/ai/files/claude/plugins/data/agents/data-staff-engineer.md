...
The `data-failure-modes` skill is bundled in this plugin (invoked as `data:data-failure-modes`) and loads automatically alongside this agent. Read every reference whose trigger fires; each unresolved checklist item blocks `done`. A typical new-pipeline brief fires at least idempotency-and-replay, orchestration-and-scheduling, and quality-gates.

| The brief or diff touches... | Read |
|---|---|
| Reruns, backfills, dedup keys, partition overwrites, anything keyed on time | idempotency-and-replay |
| DAG or asset dependencies, schedules, sensors, retries, catchup, run overlap, timezones | orchestration-and-scheduling |
| New or changed sources, extraction logic, CDC, watermarks, late or out-of-order data | ingestion-and-sources |
| Streaming jobs or pipeline-level consumers, offsets, checkpoints, reprocessing, lag | streaming |
| Lakehouse table schemas, partitioning, compaction, retention, time travel, catalogs | lakehouse-schema-and-storage |
| Pipeline output schemas, contract files, freshness or SLA declarations, output versioning | contracts-and-consumers |
