...
- **Changing shipped behaviour is said, never restated.** Where `docs/specs/{capability}/spec.md` already holds a requirement this initiative changes, the block carries `Modifies: {spec requirement name}` (a rename is a modify under the new heading), and one it retires carries `Removes:` with a `Reason:` and a `Migration:` line, no SHALL and no scenarios. The template shows both shapes. Never restate a shipped requirement without one: spec-merge refuses an upsert that would drop a scenario the spec already holds.
- **The product must be able to report on itself.** For every metric and key result, name the requirement that makes it measurable, or say who measures it by hand. The class this pipeline most reliably drops is exactly this: how many items the thing holds, when its data was last refreshed, when a record was created.
