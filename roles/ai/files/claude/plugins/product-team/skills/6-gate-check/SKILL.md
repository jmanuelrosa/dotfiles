...
A **verifier, not a generator**. Run the DoR checklist against every story and write `06-dor-report.md`. The report fails loudly: any FAIL blocks `/product-team:7-push-to-board`, and this skill never fixes what it finds (maker/checker).
...
- A closing verdict: `ALL PASS - ready for /product-team:7-push-to-board` or `N stories FAIL - stage 7 is blocked`, plus which skill to re-run (`/product-team:5-decompose` for slicing problems, manual edits + re-run `/product-team:6-gate-check` for wording).
