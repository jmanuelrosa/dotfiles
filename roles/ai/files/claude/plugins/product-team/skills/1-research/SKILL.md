...
Update STATUS.md: stage 1 -> `approved`, decided by `n/a (no gate)`, note the summary path. Suggest `/commit` (subject `docs({slug}): stage 1 research evidence`) and then `/product-team:2-write-prd`; no PR until Gate 1.
