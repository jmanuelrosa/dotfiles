...
The `sre-failure-modes` skill is bundled in this plugin (invoked as `sre:sre-failure-modes`) and loads automatically alongside this agent. Read every reference whose trigger fires; each unresolved checklist item blocks `done`. A typical SLO-and-alerting brief fires at least slos-and-error-budgets, alert-rules, and metrics-and-cardinality.

| The brief or diff touches... | Read |
|---|---|
| SLI definitions, SLO targets and windows, error budgets, burn-rate math, SLO tooling configs | slos-and-error-budgets |
| Alert rule expressions, thresholds, `for` durations, burn-rate alerts, recording rules | alert-rules |
| Routing trees, receivers, severity labels, inhibition rules, silences, escalation policies, on-call load | alert-routing-and-oncall |
| Dashboard JSON or jsonnet, panels, queries, template variables, dashboard provisioning | dashboards-as-code |
| New or changed metrics, labels, histogram buckets, metric naming, anything about cardinality or metric cost | metrics-and-cardinality |
| Trace sampling config, context propagation setup, structured log schema, PII scrubbing, retention settings | tracing-and-logging |
| Collector or agent configs, exporters, receivers, processors, pipeline wiring, telemetry egress | telemetry-pipelines |
| Runbooks, operational docs, alert annotations, automation scripts replacing manual steps | runbooks-and-automation |
| Postmortems, incident timelines, severity classification, action items, on-call handoff docs | incidents-and-postmortems |
