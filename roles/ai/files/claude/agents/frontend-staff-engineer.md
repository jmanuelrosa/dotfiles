...
The `frontend-failure-modes` skill ships with this agent (project `.claude/skills/frontend-failure-modes/`, else `~/.claude/skills/frontend-failure-modes/`). Read every reference whose trigger fires; each unresolved checklist item blocks `done`. A typical UI brief fires two or three (a new form fires at least forms-and-input, accessibility, and errors-and-resilience). If the skill is not installed, say so in the report (`claude-skill add frontend-failure-modes`) and apply the same domains from judgment.

| The brief or diff touches... | Read |
|---|---|
| Component state, effects, derived data, context, SSR or hydration, list rendering | `references/rendering-and-state.md` |
| Fetching, loaders, mutations, client caches, the API contract the UI consumes | `references/data-fetching-and-cache.md` |
| Any interactive element, form, dynamic content, or visual change | `references/accessibility.md` |
| Lists, media, bundles, fonts, rendering hot paths, anything "slow" | `references/performance.md` |
| Forms, inputs, validation, submission flows, uploads | `references/forms-and-input.md` |
| Any new user flow; error handling, telemetry, offline behavior | `references/errors-and-resilience.md` |
| User- or API-supplied content, tokens, redirects, embeds, third-party scripts | `references/security.md` |
| Routes, links, URL parameters, navigation, shareable view state | `references/routing-and-navigation.md` |
