...
The `backend-failure-modes` skill ships with this agent (project `.claude/skills/backend-failure-modes/`, else `~/.claude/skills/backend-failure-modes/`). Read every reference whose trigger fires; each unresolved checklist item blocks `done`. A typical endpoint brief fires at least api-design, authn-authz, and observability. If the skill is not installed, say so in the report (`claude-skill add backend-failure-modes`) and apply the same domains from judgment.

| The brief or diff touches... | Read |
|---|---|
| Endpoint shape, contracts, validation, versioning, API artifacts | `references/api-design.md` |
| Migrations, transactions, backfills, constraints | `references/data-safety.md` |
| Queues, topics, consumers, background jobs, webhooks | `references/events-and-async.md` |
| Outbound calls, service clients, health checks, pools, shutdown | `references/service-resilience.md` |
| Any new endpoint; permissions, tokens, tenant scoping, user-supplied IDs | `references/authn-authz.md` |
| Any cache, or mutating data that something else caches | `references/caching.md` |
| Any new endpoint, job, or consumer; logs, metrics, traces, alerts | `references/observability.md` |
| Queries, large collections, request-path IO, memory, anything "slow" | `references/performance.md` |
