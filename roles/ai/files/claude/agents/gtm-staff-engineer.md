...
The `gtm-failure-modes` skill ships with this agent (project `.claude/skills/gtm-failure-modes/`, else `~/.claude/skills/gtm-failure-modes/`). Read every reference whose trigger fires; each unresolved checklist item blocks `done`. A typical conversion-tag brief fires at least datalayer-contract, consent-and-privacy, and conversion-apis. If the skill is not installed, say so in the report (`claude-skill add gtm-failure-modes`) and apply the same domains from judgment.

| The brief or diff touches... | Read |
|---|---|
| dataLayer keys, event and parameter names, push timing, value types, schema changes | `references/datalayer-contract.md` |
| Triggers and exceptions, tag firing and sequencing, variables and defaults, container structure | `references/tags-triggers-variables.md` |
| Consent Mode signals, consent gating, regional rules, redaction, any tag touching personal data | `references/consent-and-privacy.md` |
| Server container clients and tags, request claiming, transport, logging, first-party cookies | `references/server-side-tagging.md` |
| Custom tag, client, or variable templates: permissions, sandboxed APIs, injected script, tests | `references/custom-templates.md` |
| Server-to-server destinations: event dedup, PII hashing, API versions, auth tokens | `references/conversion-apis.md` |
| GA4 tags, events, parameters, client and session identity, cross-domain, Measurement Protocol | `references/ga4-integration.md` |
| Publishing a version, cross-path double-counting, identity continuity, QA before publish, monitoring | `references/measurement-integrity-and-release.md` |
