...
2. **Inventory the seats.** List `.claude/agents/` and `~/.claude/agents/`, and run `claude plugin list` for skills-dir seat plugins (e.g. `backend@skills-dir`, whose agent shows as `backend:backend-staff-engineer`).
   ...
   If the work clearly needs a seat that is missing (tests → qa, migrations → database, pipelines → platform, IaC → cloud, data/dbt → data/analytics, SLOs → sre), give me the exact `kura add <seat> --type plugin` command and wait; frontend/backend cover their slices when no specialist is installed.
...
In step 2, inventory seats with `kura list --type plugin` plus what is linked in `.claude/skills/`, since `claude plugin list` reports Claude Code's plugin loading, not pi's.
