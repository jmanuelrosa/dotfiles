...
The living exemplars are `backend-staff-engineer.md` and `platform-staff-engineer.md`; read them before this file means anything.
...
6. `## Step 3: Open the failure-mode checklists`: names the paired skill and its lookup paths, the "typical brief fires..." example, the not-installed fallback, then the trigger table, byte-identical to the skill's SKILL.md table (see `coherence-rules.md`).
