...
4. If the report lists adoption opportunities or proposed new artifacts, ask me which to
   ...
   how in one line). Skip the question entirely when there are none.
5. Act on each accepted item here in the main conversation: config changes directly
   (settings.json, frontmatter, rule files); new skills via /skill-writer; settings/hooks
   via the update-config skill. Show what changed. If a changed file is a symlink into my
   dotfiles repo, say so and offer /commit there (new locally authored artifacts belong in
   that repo's registries). Drop declined items without ceremony.

The subagent never modifies files. In this conversation, modify files only for
items I explicitly accepted in step 4.
