...
text. `NO_COLOR` disables it and `FORCE_COLOR` forces it on, with `NO_COLOR` winning. This is the
one deliberate divergence from `claude-skill`, which always emits escape codes because fish's
`set_color` does.
...
Would record in /Users/you/work/api/.claude/claude-kit.json:
...
These two are the only environmental inputs, which is what makes a test run hermetic.
...
✓ Linked 'coderabbit' into /Users/you/work/api/.claude/skills
```
...
$ claude-kit adopt
Recorded in /Users/you/api/.claude/claude-kit.json:
  skill 'context-engineering'      dep-of:spec-driven-development
  skill 'spec-driven-development'  direct
...
Set `FORCE_COLOR=1` to keep it, or `NO_COLOR=1` to switch it off everywhere. `claude-skill`
always emits codes, because fish's `set_color` does; this is the one place the two differ on
purpose.
...
Both. Bare `--group` gives the grouped view, matching `claude-skill list --group`. `--group <tag>`
narrows to one tag, which claude-skill cannot do.
