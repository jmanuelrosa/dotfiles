...
This file is the canonical one and every agent reads it under the name it expects, so it names skills, agents and commands by concept rather than by the spelling any one harness gives them.
Each harness's own mechanics (settings, hooks, plan mode, sandbox, command names) live in that harness's adapter, beside this file rather than in it.
...
- A sandboxed ctx7 dies with a causeless `✖ fetch failed`; how each harness exempts it, if at all, is in that harness's adapter. When ctx7 fails to reach the API rather than returning no match, name the failure and fall back to WebSearch/WebFetch, never to training data.
...
- Never use em or en dashes in chat. Use a regular hyphen, comma, colon, or parentheses. (A hook catches them in files; conversation is the surface no hook sees.)
...
- **Send broad searching to a subagent.** A read-only search agent for locating code across unknown files, a general-purpose one for a multi-step hunt. A subagent's reading is discarded when it returns, so the main thread pays only for the conclusion.
...
Offer, in this order: finish the current thread, write the state down (the `handoff` skill exists for this), then start a fresh session.
