...
`ux-shaper` is a standalone global agent provisioned by the AI role, not bundled in this plugin. If it is unavailable, stop and tell the human to run `make run-role ROLE=ai`, then re-run this stage. With `roster.ux_spec: inline` you write the spec yourself against the template and say so in it; there is no setting that skips the artifact, because "no UI" asserted in passing is how a surface with three unhandled states reaches implementation.
