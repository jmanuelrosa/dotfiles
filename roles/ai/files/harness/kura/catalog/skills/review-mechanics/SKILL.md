...
Report nothing in: generated or vendored trees, lockfiles, snapshots and fixtures asserting only shape, and `roles/ai/files/harness/kura/catalog/skills/<upstream>` trees in the dotfiles repo, since `kura update` replaces those wholesale and a finding there is discarded on the next sync.
