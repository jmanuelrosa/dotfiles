...
`--worktree` checks the branch out in a sibling directory through the `shell` role's `wt add` instead of in the current working tree, so a ticket can be started without putting the checkout you are in aside. The directory is named after the ticket alone (`PROJ-123` → `../proj-123`, GitHub 456 → `../gh-456`): the branch already carries the title, and this is a name to recognise in a path and in a herdr tab. `--worktree=DIR` overrides it; fish's optional-argument parsing wants the value attached, so `--worktree DIR` would read `DIR` as the issue.

**The current working tree is never touched.** The branch is cut with `git branch --no-track` off `origin/<default>`, which is up to date without a pull and works with a dirty tree, and `wt add` is what checks it out. `--no-track` matters because a branch started from a remote-tracking ref inherits it as upstream, and until the first `push -u` that upstream is the default branch. On the GitHub path `gh issue develop` runs without `--checkout` for the same reason, and the new ref is fetched so `wt add` creates a local branch tracking it. A branch that already has a worktree is reused rather than re-added.

A script is its own process and cannot `cd` the shell that ran it, so landing in the worktree is left to the two things that can: inside a herdr session it passes `--herdr --focus` to `wt add`, which opens the worktree as a workspace and moves you to it, and everywhere else the closing line prints the path to `cd` into. `wt add` also carries the untracked local setup across (`.env*`, `.vscode/`, `.claude/`, and a frozen-lockfile install), which is the reason to go through it rather than calling `git worktree add`.
