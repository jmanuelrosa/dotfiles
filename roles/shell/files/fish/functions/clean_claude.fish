function clean_claude --description "Remove Claude Code artifacts under a directory tree: skills, agents, whole .claude projects, or everything on the machine"
  ...
  if not command -q fd
    echo "❌ clean_claude needs fd (brew install fd)."
    return 1
  end
  if contains -- $mode project purge; and not command -q claude
    echo "❌ clean_claude $mode needs the claude CLI to purge project state."
    return 1
  end
  ...
  if test $mode = project; and not command -q jq
    echo "⚠️  jq is missing, so subprojects' ~/.claude.json entries will be left in place."
  end
  ...
  if test $mode = purge
    ...
    echo "ℹ️  Skipping "(_clean_claude_pretty "$global_hit")" (global config). Use 'clean_claude purge' for that."
    set -e global_hit
  end
  ...
  if test (count $targets) -eq 0; and test $mode != project
    echo "✨ No $label under "(_clean_claude_pretty "$root")" outside dependency trees."
    return 0
  end

  if test (count $targets) -gt 0
    echo "🔎 "(count $targets)" $label under "(_clean_claude_pretty "$root")":"
    for target in $targets
      set -l note ""
      _clean_claude_tracked "$target"; and set note "  ⚠️  git-tracked"
      test "$target" = "$HOME/.claude"; and set note "  ⚠️  global config"
      echo "   • "(_clean_claude_pretty "$target")"$note"
    end
  else
    echo "🤷 No $label under "(_clean_claude_pretty "$root")"."
  end
  ...
  if test $mode = project
    echo "🗂  Purging Claude state under "(_clean_claude_pretty "$root")": transcripts, tasks and file history for the whole tree, plus the ~/.claude.json entry of each project at or below it."
  else if test $mode = purge
    echo "🗂  Purging Claude state for EVERY project on this machine."
  end
  ...
  if test $mode = purge
    echo "⚠️  "(_clean_claude_pretty "$HOME/.claude")" goes with it: credentials, session history, plugins"
    echo "   and the skills/agents symlinks the ai role owns. Getting back means"
    ...
    if test $mode = project
      if test (count $targets) -eq 0
        set prompt "Purge Claude state under "(_clean_claude_pretty "$root")"?"
      else
        set prompt "Remove "(count $targets)" $label and purge Claude state under "(_clean_claude_pretty "$root")"?"
      end
    end
  ...
  if test $removed -ne (count $targets)
    echo "⚠️  Removed $removed of "(count $targets)" $label; the rest survived (permissions?)."
    return 1
  end

  switch $mode
    case skills agents
      echo "✨ Removed $removed $label. Restore what you need with 'claude-skill add' / 'claude-agent add'."
    case project
      if test $removed -eq 0
        echo "✨ Purged Claude state under "(_clean_claude_pretty "$root")"; there was no $label to remove."
      else
        echo "✨ Removed $removed $label and purged Claude state under "(_clean_claude_pretty "$root")"."
      end
    case purge
      echo "✨ Removed $removed $label and purged all project state. Run 'make run-role ROLE=ai' to rebuild the managed links."
  end
end
