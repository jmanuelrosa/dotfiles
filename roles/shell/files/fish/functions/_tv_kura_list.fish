function _tv_kura_list --description "Television source: list catalog skills or agents with groups and link status" --argument-names kind filter
    ...
    switch $filter
        ...
        case '' all
            set select '.'
        case '*'
            _ui err "_tv_kura_list: filter must be 'linked', 'available', 'noglobal', or empty"
            return 1
    end
...
function _tv_kura_fmt --description "Format one television row: name, groups, link status"
