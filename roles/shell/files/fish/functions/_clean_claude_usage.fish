function _clean_claude_usage --description "Print clean_claude usage"
  ...
  echo "  -i, --include PATTERN  stop skipping a default-excluded name, e.g. --include vendor"
  ...
  echo "Dependency, cache and build trees (node_modules, vendor, deps, dist, Library, ...) are"
  echo "always skipped; CLEAN_CLAUDE_EXCLUDES extends that list permanently. Monorepo"
  echo "workspaces (packages/, apps/) are NOT skipped, so their .claude dirs are candidates."
