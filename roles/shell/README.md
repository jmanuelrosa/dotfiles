...
- `clean_pi [MODE] [ROOT] [--dry-run] [--exclude PATTERN] [--include PATTERN]` uses the same tree exclusions, preview and confirmation as `clean_claude`, but targets Pi's `.pi` and neutral `.agents` project directories. `project` also deletes sessions under the configured Pi session directory whose JSONL header records a cwd at or below `ROOT`; sessions stored via `--session-dir` or nested-project settings outside that directory, and entries in global `trust.json`, are left alone. `skills` removes `.pi/skills` and `.agents/skills`; `agents` removes `.pi/agents` and `.agents/agents`. `purge` removes project directories under `$HOME`, Pi's global agent directory (credentials, sessions, packages, settings) and `~/.agents/skills`, leaving other shared `~/.agents` content intact; it requires typing `purge` and rebuilding the AI role plus re-authentication afterward. Non-purge modes never remove global Pi config or global neutral skills.
- `clean:ai` chains `clean:claude` and `clean:pi`, stopping if the first fails. All three alias families have `:skill` (also `:skills`), `:agents` and `:purge` variants. Each cleaner previews and confirms separately, so the combined purge requires confirming each harness. Fish forwards options to both; Pi accepts but ignores `--worktree-config`, which applies only to Claude because Pi session deletion is based on the session header's cwd.
