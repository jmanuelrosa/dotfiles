...
Executables in `roles/ai/files/scripts/` are **glob**-symlinked into `~/.local/bin` by [roles/ai/tasks/main.yml](roles/ai/tasks/main.yml), so dropping a file there is the whole install step and editing it takes effect without re-running the playbook. `roles/work/files/scripts/` (`s-task`, `s-db`) works the same way but is gated to the work profile.

Two ship today. [claude-kit](roles/ai/files/scripts/claude-kit) is the registry-driven CLI: `add` installs a skill, agent or plugin by name with the scope decided by the `global` tag, and `scout` reads the same catalogue from the other end, fingerprinting the current project and ranking the skills it does not already have. [weekly-recap](roles/ai/files/scripts/weekly-recap) aggregates Jira, GitHub and GitLab activity into markdown. The direction of travel is that `claude-kit` absorbs the `claude-skill` / `claude-agent` fish functions, so new tooling of this kind lands as a `claude-kit` subcommand rather than a new fish function.

Conventions, all load-bearing: `#!/usr/bin/env python3` with **stdlib only** (no `jq` dependency, matching the hooks); the repo located by searching for the `dotfiles.yml` marker with `DOTFILES_DIR` as the override seam, never a relative path; `HOME`, `DOTFILES_DIR` and the cwd as the only environmental inputs, which is what makes the [tests/](tests/) `kit` fixture able to isolate a run; and a named exit-code table so tests assert on codes rather than wording. A script needing third-party packages skips the shebang and declares PEP 723 inline metadata for `uv run` instead.

**A deterministic pipeline belongs in a script, not a skill.** `weekly-recap` and `skill-scout` were both skills until their prose turned out to describe pure mechanism (query, filter, group, format) with no judgment in it; as scripts they cost no tokens, run outside Claude, and are covered by pytest. Reach for a skill only when the work genuinely needs a model's judgment.

**Split the difference when a skill has a mechanical half worth extracting.** Four skills do, and they share one shape: a `context` script gathers everything the drafting step needs in a single round trip, and an `apply` script performs the outward action after the skill's approval gate. `commit` keeps its drafting prose and delegates to `scripts/context.sh` plus `scripts/apply.py`. `pr` does the same with `scripts/context.py` (host, base branch, template, diff, and the fully deterministic title and scope derivation) plus `scripts/apply.py`. `coderabbit` puts the paginated GraphQL thread fetch, its idempotency filter and its noise stripping in `scripts/context.py`. `jira` emits its ADF description from `scripts/adf.py`, which is also where the format's hard-won constraints live (acli rejects a text node carrying `code` beside `strong` or `em` as `INVALID_INPUT`). What stays in prose every time is the judgment: what to write, which boxes the diff actually supports, whether a review comment is real. These scripts follow the same conventions as the standalone ones above, minus the `dotfiles.yml` marker search, since a skill directory is symlinked standalone into a project and cannot assume the repo is reachable.

**An `apply.py` is a hole in the git gate unless it is declared.** [git-skill-gate.sh](roles/ai/files/claude/hooks/git-skill-gate.sh) sees the Bash command, not the subprocess it spawns, so a script that pushes carries no `git push` token to match on. `WRAPPER_SCRIPTS` maps each such script's path to the subcommand it stands in for, which is what keeps `/commit` and `/pr` gating their own script and not each other's. Any new script that commits, pushes or opens a PR has to be added there, and it should re-implement the hook's hard blocks itself (attribution lines, typographic dashes, `.claude/tasks/` paths, cleartext secrets) because the hook can no longer read the message it is about to write. [tests/test_git_skill_gate.py](tests/test_git_skill_gate.py) covers both directions, including that reading such a script is not running it.
